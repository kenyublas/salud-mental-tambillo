...
function CerebroIcon({ activo, hablando }) {
  return (
    <svg viewBox="0 0 64 64" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Ondas de actividad cuando habla */}
      {hablando && (
        <>
...
function Burbuja({ msg }) {
  ...
  return (
    <div className={`flex ${esUser ? 'justify-end' : 'justify-start'} mb-2`}>
      {!esUser && (
        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex-shrink-0 mr-2 flex items-center justify-center text-xs">
          🧠
        </div>
      )}
...
export default function CerebroFlotante() {
  ...
  // Cargar contexto del sistema al montar
  useEffect(() => {
...
Últimos pacientes: ${registros.slice(0, 3).map(p => p.nombres).filter(Boolean).join(', ')}`;
  ...
  useEffect(() => {
    if (abierto && !saludado && contexto) {
      setSaludado(true);
      enviarMensaje('Hola, dame un resumen del día', true);
    }
    ...
    <>
        ...
        <div className="fixed bottom-24 right-4 z-50 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-purple-100 flex flex-col overflow-hidden"
          ...
          <div className="bg-gradient-to-r from-purple-500 to-pink-500 px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8">
                <CerebroIcon activo={true} hablando={cargando} />
              </div>
              <div>
                <p className="font-bold text-white text-sm" style={{ fontFamily: 'Poppins, sans-serif' }}>Dr. Mind</p>
            ...
            {historial.length === 0 && !cargando && (
              <div className="text-center text-gray-400 py-8">
                <div className="w-16 h-16 mx-auto mb-3">
                  <CerebroIcon activo={false} hablando={false} />
                </div>
                <p className="text-xs font-semibold">Hola Lic. Janeth 👋</p>
                <p className="text-xs mt-1">Puedes preguntarme sobre tus pacientes, citas o estadísticas</p>
              </div>
            )}
            {historial.map((msg, i) => <Burbuja key={i} msg={msg} />)}
            {cargando && (
              <div className="flex justify-start mb-2">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex-shrink-0 mr-2 flex items-center justify-center text-xs">🧠</div>
          ...
          {historial.length <= 1 && !cargando && (
            <div className="px-3 py-2 flex gap-1.5 flex-wrap border-t border-gray-100">
              {['¿Cuántos pacientes hoy?', '¿Citas pendientes?', 'Resumen del mes'].map(s => (
      ...
      <button
        ...
        style={{
          ...
          boxShadow: abierto
            ? '0 0 30px rgba(168,85,247,0.6), 0 8px 32px rgba(0,0,0,0.2)'
            : '0 8px 32px rgba(0,0,0,0.15)',
        }}
        title="Dr. Mind — Asistente IA"
      >
